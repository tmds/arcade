# Arcade Infrastructure for Publishing

This document describes the YAML based approach provided by Arcade for build assets publishing. This infrastructure use [YAML stages](https://docs.microsoft.com/en-us/azure/devops/pipelines/process/stages?view=azure-devops&tabs=yaml) and is tightly integrated with [Maestro's Channels](https://github.com/dotnet/arcade/blob/master/Documentation/BranchesChannelsAndSubscriptions.md) concept.

## What are YAML stages?

Stages are a concept introduced by Azure DevOps to organize the jobs in a pipeline. Just as Jobs are a collection of Steps, Stages can be thought about as a collection of Jobs, where for example, the same pipeline can be split into Build, Test, Publishing, and deployment stages. Stages are the way that Azure DevOps is bringing build and release pipelines together, and are going to be the replacement for the Release Management UI based release pipelines. The official documentation for YAML Stages can be found [here](https://docs.microsoft.com/en-us/azure/devops/pipelines/process/stages?view=azure-devops&tabs=yaml).

The publishing infra uses stages to represent Maestro's Channels. Therefore, there should be a 1:1 correspondence between Maestro's channels and build publishing stage(s). However, the infra also  adds [stages for validating the assets](https://github.com/dotnet/arcade/blob/master/eng/common/templates/post-build/post-build.yml#L25), for instance Signing Validation, NuGet metadata validation and SDL validation. These publishing stages are referred to as `post-build` stages and their implementation can be [found here](https://github.com/dotnet/arcade/tree/master/eng/common/templates/post-build).
...
* Support test, publishing and/or asset validation errors to be reported in the same UI as the build errors.
* Additional stages can be added after the publishing ones allowing for extensibility to the default Arcade publishing.
...
1. Add an artifact category (see explanation below) for assets produced by the build:

    * Most repositories will use `.NETCore` as the category unless assets should be published to a feed other than `dotnet-core`. See [Advanced Scenarios](##overriding-the-publishing-feed-used-for-builds-in-the-dev-channel) for instructions on how to publish to a different feed.
...
**If you have a direct reference to `job.yml`:**
...
Since the post-build stages will only trigger during builds that run in the internal project, there are some additional steps that need to be performed in order to test that the changes to the pipeline are correct, and that publishing works as expected.
